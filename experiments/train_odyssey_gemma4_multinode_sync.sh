#!/bin/bash
# Two-node synchronous fused-agent training launcher for Gemma4 E4B on current slime.
#
...
usage() {
   cat <<'EOF'
Usage:
  bash experiments/train_odyssey_gemma4_multinode_sync.sh [options]

Options:
  --harness NAME                         Fused prompt harness: bare, cot, react, gem, unified_gem.
  --model PATH                           HF Gemma4 model path. Default: /share/nlp/share/plm/gemma-4-E4B-it.
  ...
  --yarn-original-max-position-embeddings N
                                         Native model context used by YaRN. Default: 32768.
  ...
  --retrieval-concurrency N              Concurrent retrieval requests. Default: 512.
  ...
  --grm-max-input-tokens N               Maximum GRM input content tokens. Default: 65536.
...
# Alternate training and rollout across all eight GPUs per node. The pinned
...
if [ ! -x "${RAY_BIN}" ]; then
   echo "Ray executable does not exist: ${RAY_BIN}" >&2
   exit 2
fi
...
if is_truthy "${ENABLE_USE_GRM_TRAIN}" || is_truthy "${ENABLE_USE_GRM_EVALS}" || [ "${CUSTOM_RM_PATH:-}" = "${GRM_CUSTOM_RM_PATH}" ]; then
   ...
   if [ -z "${OPENROUTER_API_KEY:-}" ]; then
      echo "GRM evals require OPENROUTER_API_KEY (or OPENAI_API_KEY with GRM_BASE_URL/OPENAI_BASE_URL)." >&2
      exit 2
   fi
fi
...
# Colocated training and rollout use all eight GPUs on both nodes while
# retaining Gemma4's TP and PP constraints.
...
if [ "${CP_SIZE}" -lt 1 ] || [ $((ACTOR_GPUS % CP_SIZE)) -ne 0 ]; then
   echo "ACTOR_GPUS=${ACTOR_GPUS} must be divisible by positive CP_SIZE=${CP_SIZE}." >&2
   exit 2
fi
...
# Keep enough candidate groups queued to select a stable post-filter batch.
# This mirrors the Qwen3 synchronous collector: candidates are retained per
# family, then the final quota selection can prefer higher-mean-step groups.
...
# replay metadata path does not yet support the current SGLang response format.
TOP_P="${TOP_P:-1.0}"
TOP_K="${TOP_K:-64}"
...
# A zero-variance group must never reach the policy-gradient update. Keep this
# strict even if a stale environment variable asks the collector to relax.
...
# A distributed checkpoint can reshard model tensors, but exact training-state
# resume requires the same parallel topology. Refuse a partial resume instead of
...
PERF_ARGS=(
   # Gemma4 E4B uses TP1 for both training and rollout.
...
OPTIMIZER_ARGS=(
   ...
   # Eight colocated ranks otherwise pin both bf16 and fp32 optimizer copies
   # concurrently during initialization, which exceeds the CUDA host-allocation
   # limit on this node. Pageable CPU offload is slower but avoids that failure.
   --no-pin-cpu-params
   --no-pin-cpu-grads
...
if is_truthy "${CHECK_WEIGHT_UPDATE_EQUAL:-0}"; then
   # This destructive debug check resets every SGLang tensor. Gemma4's trainer
   # only owns the text model, so frozen vision/audio tensors and fixed text
   # buffers cannot be restored by the subsequent actor weight update.
...
CLUSTER_NO_PROXY="localhost,127.0.0.1,0.0.0.0,${MASTER_ADDR}"
...
export RUNS_ROOT="${RUNS_ROOT:-/share/nlp/share/gem/runs}"
export RUN_ROOT="${RUN_ROOT:-${RUNS_ROOT}/${EXPERIMENT_NAME}}"
export MCP_ENV_ROOT="${MCP_ENV_ROOT:-${RUN_ROOT}/cache/mcp_envs}"
...
echo "Rollout timeouts: trajectory=${FUSED_TRAJECTORY_TIMEOUT}s, group=${SLIME_ROLLOUT_GROUP_TIMEOUT}s, eval_trajectory=${FUSED_EVAL_TRAJECTORY_TIMEOUT}s, infra_retries=${ROLLOUT_INFRA_RETRY_TIMES}"
...
echo "Dynamic filter: enable=${ENABLE_DYNAMIC_SAMPLING_FILTER}, path=${DYNAMIC_SAMPLING_FILTER_PATH}, strict=true, relax_after_groups=${FULLY_ASYNC_FILTER_RELAX_AFTER_GROUPS}; webqa_min_unique_searches=${FUSED_WEBQA_MIN_UNIQUE_SEARCHES}, webqa_reward_match=${FUSED_WEBQA_REWARD_MATCH_MODE}"
...
MASTER_NODE_IP="$(getent ahostsv4 "${MASTER_ADDR}" | awk 'NR == 1 {print $1}')"
...
raw = sys.argv[2]
devices = raw.split(",")
if len(devices) != expected or any(not device.isdigit() for device in devices):
    raise SystemExit(f"Ray head CUDA_VISIBLE_DEVICES must contain exactly {expected} numeric GPU indices; got {raw!r}")
...
if "${RAY_BIN}" job list --address="${RAY_DASHBOARD_ADDRESS}" >/dev/null 2>&1; then
   ...
   RAYLET_CUDA_VISIBLE_DEVICES=""
   if [ -n "${RAYLET_PID}" ]; then
      RAYLET_CUDA_VISIBLE_DEVICES="$(tr '\0' '\n' < "/proc/${RAYLET_PID}/environ" | sed -n 's/^CUDA_VISIBLE_DEVICES=//p' | tail -n 1)"
   fi
   if [ "${RAYLET_CUDA_VISIBLE_DEVICES}" != "${MASTER_CUDA_VISIBLE_DEVICES}" ]; then
      echo "Existing Ray head has CUDA_VISIBLE_DEVICES=${RAYLET_CUDA_VISIBLE_DEVICES:-<unset>}, expected ${MASTER_CUDA_VISIBLE_DEVICES}." >&2
      echo "Restart it with 'ray stop --force' before launching so Ray uses the requested eight-GPU visibility." >&2
      exit 2
   fi
   echo "Reusing existing Ray head at ${RAY_DASHBOARD_ADDRESS}"
else
   echo "No local Ray head detected; starting one on ${MASTER_ADDR} (${MASTER_NODE_IP})"
   "${RAY_BIN}" stop --force >/dev/null 2>&1 || true
   CUDA_VISIBLE_DEVICES="${MASTER_CUDA_VISIBLE_DEVICES}" "${RAY_BIN}" start --head \
...
def identities(host):
    values = {host.lower(), host.split(".", 1)[0].lower()}
    try:
...
while True:
    ...
    alive_identities = [
        identities(node.get("NodeManagerHostname") or node.get("NodeManagerAddress", ""))
...
CLUSTER_ARGS=(
   ...
   # Keep rollout port allocation aligned with Ray's eight visible GPU slots.
...
start_managed_eval_serper
"${RAY_BIN}" job submit --address="${RAY_DASHBOARD_ADDRESS}" \
...
if [ "${RAY_JOB_WAIT:-0}" != "1" ] && [ "${RAY_JOB_FOLLOW_LOGS:-1}" = "1" ]; then
   ...
   "${RAY_BIN}" job logs --address="${RAY_DASHBOARD_ADDRESS}" --follow "${RAY_SUBMISSION_ID}"
fi
