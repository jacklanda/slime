...
def local_search_schema() -> dict:
    return tool_schema(
        "local_search",
        "Search for information using a dense retrieval server with Wikipedia corpus, and use a LLM to summarize the search results.",
...
def system_prompt() -> str:
    tools = json.dumps([local_search_schema()], indent=2, ensure_ascii=False)
    return (
...
async def run_search(action: ToolCall, *, retrieval_url: str, max_results: int) -> tuple[str, dict[str, Any]]:
    ...
    random.shuffle(urls)
    retry_budget = min(max(1, int(os.environ.get("CUT_BILL_RETRIEVAL_MAX_RETRIES", "2"))), len(urls))
    ...
    if os.environ.get("CUT_BILL_USE_REFINE", "0").lower() in {"0", "false", "no", "off"}:
...
def _format_original_results(data: Any, *, max_results: int) -> str:
    ...
    max_chars = max(1, int(os.environ.get("CUT_BILL_MAX_CONTENT_LENGTH", "4000")))
    documents = []
...
async def _refine(query: str, documents: str) -> str | None:
    configured = os.environ.get("CUT_BILL_REFINE_SERVER_URL") or os.environ.get("REFINE_SERVER_URL")
    if not configured:
        raise RuntimeError(
            "cut_bill requires CUT_BILL_REFINE_SERVER_URL (comma-separated OpenAI-compatible base URLs), "
            "or explicitly set CUT_BILL_USE_REFINE=0"
        )
    urls = [url.strip().rstrip("/") for url in configured.split(",") if url.strip()]
    attempts = max(1, int(os.environ.get("CUT_BILL_REFINE_MAX_RETRIES", "3")))
    payload = {
        "model": os.environ.get("CUT_BILL_REFINE_MODEL", "Qwen/Qwen3-8B"),
    ...
    for _ in range(attempts):
        ...
        try:
            ...
            summary = parse_refine_response(data["choices"][0]["message"]["content"])
