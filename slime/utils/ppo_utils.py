...
def _split_replicated_loss_gather_grad(
    ...
    *,
    ...
    return torch.narrow(
        ...
        length=local_last_dim,
    ).contiguous()


class _ReplicatedLossAllGatherLastDim(torch.autograd.Function):
    """Gather TP vocab shards without multiplying the replicated loss gradient."""
...
def _gather_true_on_policy_full_logits(
    ...
    *,
    ...
    _, world_size = _get_vocab_parallel_rank_size(process_group)
    if world_size <= 1:
        return _prepare_true_on_policy_full_logits(logits, vocab_size=vocab_size)
...
def _calculate_log_probs_and_entropy_true_on_policy(
    ...
    *,
    ...
    """Compute the replicated full-vocabulary loss used by SGLang parity mode."""
    if logits.size(0) == 0:
    ...
    entropy = None
    if with_entropy:
        entropy_log_probs = log_probs_full if with_entropy_grad else log_probs_full.detach()
    ...
    *,
    ...
    if true_on_policy:
        if log_prob_keep_mask is not None:
            raise ValueError("True-on-policy logprobs do not support a top-p keep mask")
